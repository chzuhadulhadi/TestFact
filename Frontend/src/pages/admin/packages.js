...
import { Button, Table, TableBody, td, TableHead, Typography, TableRow, TableContainer, Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem, InputLabel, Select, FormControl, Grid, Modal } from '@mui/material';
import { apiCall } from '../../apiCalls/apiCalls';
import { getPackage, deletePackage, updatePackage, createPackage } from '../../apiCalls/apiRoutes';
...
function Packages() {
    ...
    const filteredPackages = packages.filter((user) => {
        return (
            user.packageName.toLowerCase().includes(searchText.toLowerCase()));
    });
    console.log(filteredPackages);
    return (
        <div className='w-[90%] mx-auto'>
          <Typography className='mb-3 text-xl md:text-3xl' variant='h2' justifyContent={'center'} display={'flex'}>Package List</Typography>
            <TextField
                label="Search"
                variant="outlined"
                fullWidth
            ...
            />
            <Button className='w-1/4 my-3' onClick={() => setOpen(true)}>Create Package</Button>
            <Modal
                open={open}
            ...
            >
                <div style={{ position: 'absolute', top: '50%', left: '50%',padding: '10px', transform: 'translate(-50%, -50%)', width: '400px', bgcolor: 'white', border: '2px solid #000', boxShadow: 24, p: 4,background:'white' }}>
                   
                    <Typography id="modal-modal-description" sx={{ mt: 1 }}>
                        Package Name
                    </Typography>
                    <TextField  variant="outlined" fullWidth onChange={(e) => setPackageData({ ...packageData, packageName: e.target.value })} my={2}/>
                    <Typography id="modal-modal-description" sx={{ mt: 1 }}>
                        Package Price in $
                    </Typography>
                    <TextField variant="outlined" fullWidth onChange={(e) => setPackageData({ ...packageData, packagePrice: e.target.value })} my={2}/>
                    <Typography id="modal-modal-description" sx={{ mt: 1 }}>
                    Package Duration in days
                    </Typography>
                    <TextField  variant="outlined" fullWidth onChange={(e) => setPackageData({ ...packageData, packageDuration: e.target.value })} my={2}/>
                    <Typography id="modal-modal-description" sx={{ mt: 1 }}>
                     Number Of Tests
                    </Typography>
                    <TextField  variant="outlined" fullWidth onChange={(e) => setPackageData({ ...packageData, numberOfTests: e.target.value })} my={2}/>
                    <Typography id="modal-modal-description" sx={{ mt: 1 }}>
                     Description Lines
                    </Typography>
                    <TextField  variant="outlined" fullWidth onChange={(e) => setPackageData({ ...packageData, support: e.target.value })} my={2}/>
                ...
                <TableBody>
                    {filteredPackages.map((user) => (
                        <TableRow key={user.id}>
                            ...
                            <td>{user.packageDuration}{' days'}</td>
            ...
            <Dialog open={editPackageDialogOpen} onClose={handleCloseEditPackageDialog}  >
